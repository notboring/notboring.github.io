...
var mask_Main = function() {
	pixi_plugins_app_Application.call(this);
	this._init();
};
mask_Main.main = function() {
	new mask_Main();
};
mask_Main.__super__ = pixi_plugins_app_Application;
mask_Main.prototype = $extend(pixi_plugins_app_Application.prototype,{
	_init: function() {
		this.position = "fixed";
		this.onUpdate = $bind(this,this._onUpdate);
		pixi_plugins_app_Application.prototype.start.call(this,"recommended");
		this._bg = PIXI.Sprite.from("assets/alphamask/bkg.jpg");
		this.stage.addChild(this._bg);
		this._cells = PIXI.Sprite.from("assets/alphamask/cells.png");
	...
	,_reset: function() {
		var tmp = Math.random() * 550;
		this._target.x = Math.floor(tmp);
		var tmp1 = Math.random() * 300;
		this._target.y = Math.floor(tmp1);
	}
	,_onUpdate: function(elapsedTime) {
		this._mask.position.x += (this._target.x - this._mask.x) * 0.1;
		this._mask.position.y += (this._target.y - this._mask.y) * 0.1;
		if(Math.abs(this._mask.x - this._target.x) < 1) {
			this._reset();
		}
	}
});
