...
deviceinfo_Main.prototype = $extend(pixi_plugins_app_Application.prototype,{
	_init: function() {
		...
		txt.text = "Resolution: \t\t" + Std.string(window.innerWidth) + " x " + Std.string(window.innerHeight);
		this._info.addChild(txt);
		txt = new PIXI.Text("",{ fill : "#FFFFFF"});
		txt.text = "Pixel Ratio: \t\t" + window.devicePixelRatio;
		txt.y = 40;
		this._info.addChild(txt);
		txt = new PIXI.Text("",{ fill : "#FFFFFF"});
		var _g = this.renderer.type;
		var tmp;
		switch(_g) {
		case PIXI.RENDERER_TYPE.CANVAS:
			tmp = "Canvas";
			break;
		case PIXI.RENDERER_TYPE.UNKNOWN:
			tmp = "Unknown";
			break;
